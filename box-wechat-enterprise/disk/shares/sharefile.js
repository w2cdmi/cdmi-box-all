...
Page({
    ...
    data: {
        previewImageUrls: [],
        backIndexBtn: false  //返回首页按钮默认不显示
    },
    ...
    onShareAppMessage: function (options) {
        return {
            title: linkName,
            path: '/disk/shares/sharefile?linkCode=' + linkCode + '&forwardId=' + getApp().globalData.cloudUserId +'&enterpriseId=' + enterpriseId + '&scene=1000',
            imageUrl: shareIcon,
...
function getLinkInfo(linkCode, page) {
    linkClient.getLinkInfo(linkCode, function (data) {
        ...
        if (data.link.plainAccessCode != undefined && data.link.plainAccessCode != "" && plainAccessCode != data.link.plainAccessCode){
            wx.redirectTo({
                url: '/disk/batchShare/extractionEode/extractionEode?forwardId=' + forwardId + "&linkCode=" + linkCode + "&plainAccessCode=" + data.link.plainAccessCode + "&linkName=" + encodeURIComponent(data.link.alias) + "&enterpriseId=" + enterpriseId,
            })
            return
        }
