...
int32_t LocalFile::copy(const Path& new_parent)
{
	return RT_NOT_IMPLEMENT;
}

int32_t LocalFile::move(const Path& new_parent)
{
...
int32_t LocalFile::read(const int64_t& offset, const uint32_t size, unsigned char* buffer)
{
	...
	if (!SetFilePointerEx(file_, 
		...
		FILE_BEGIN))
	{
		HSLOG_ERROR(MODULE_NAME, GetLastError(), "seek file of %s failed.", 
			Utility::String::wstring_to_string(path_).c_str());
	...
	do
	{
		...
		{
			HSLOG_ERROR(MODULE_NAME, GetLastError(), "read file of %s failed.", 
				Utility::String::wstring_to_string(path_).c_str());
...
int32_t LocalFile::write(const unsigned char* buffer, const uint32_t size, const int64_t& offset)
{
	...
	{
		HSLOG_ERROR(MODULE_NAME, GetLastError(), "write file of %s failed.", 
			Utility::String::wstring_to_string(path_).c_str());
