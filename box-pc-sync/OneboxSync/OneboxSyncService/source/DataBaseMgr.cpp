...
public:
	...
	{
		return metaDataTable_->getVersionTable();
	}
	...
	{
		return metaDataTable_->getTransTaskTable();
	}
...
private:
	void createTables()
	{
		std::wstring parent = userContext_->getConfigureMgr()->getConfigure()->userDataPath();
		...
		long offset = userContext_->getConfigureMgr()->getConfigure()->uploadFilterPeriod()*60;

		metaDataTable_ = MetaDataTable::create(userContext_, parent+PATH_DELIMITER, offset);
		syncDataTable_ = SyncDataTable::create(parent+PATH_DELIMITER+std::wstring(SQLITE_SYNCDATA_TABLE));
	}
	...
	int32_t checkDb(const std::wstring& parent)
	{
		...
		//check task
		bool taskRebuild = false;
		try
		{
			if(SD::Utility::FS::is_exist(parent+PATH_DELIMITER+SQLITE_TASKINFO_TABLE))
			{
				db_.open(SD::Utility::String::wstring_to_utf8(parent+PATH_DELIMITER+SQLITE_TASKINFO_TABLE).c_str());
				(void)bufSQL.format("SELECT %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s FROM %s LIMIT 0,1", 
								TRANS_TASK_ROW_ID, 
		...
		catch (CppSQLite3Exception& e)
		{
			SERVICE_ERROR(MODULE_NAME, RT_SQLITE_ERROR,
						...
						e.errorCode(), e.errorMessage());
			db_.close();
			SERVICE_INFO(MODULE_NAME, RT_OK, "rebuild taskInfo.");
			CHECK_RESULT(SD::Utility::FS::remove(parent+PATH_DELIMITER+SQLITE_TASKINFO_TABLE));
			taskRebuild = true;
		}
		catch(...)
		{
		...
		if(taskRebuild&&hasDiff)
		{
			try
			{
				db_.open(SD::Utility::String::wstring_to_utf8(parent+PATH_DELIMITER+SQLITE_DIFFINFO_TABLE).c_str());
				const char* sqlStr = bufSQL.format("UPDATE %s SET %s = %d WHERE %s = %d", 
									DIFF_TABLE_NAME, 
									DIFF_ROW_STATUS, (int)Diff_Normal, 
									DIFF_ROW_STATUS, (int)Diff_Running);
				int32_t iModify = db_.execDML(bufSQL);
				SERVICE_DEBUG(MODULE_NAME, RT_OK, "restore running task, iModify:%d, sqlStr:%s", iModify, sqlStr);
				db_.close();
			}
...
DataBaseMgr* DataBaseMgr::create(UserContext* userContext)
{
	return static_cast<DataBaseMgr*>(new DataBaseMgrImpl(userContext));
}