...
public:
	...
	virtual void refreshDiff(const DiffStatus status, int errorCode = 0)
	{
		...
		userContext_->getDataBaseMgr()->getDiffTable()->refreshDiff(completeList_, Diff_Complete);
		userContext_->getDataBaseMgr()->getDiffTable()->refreshDiff(idList, status);
		...
		if(INVALID_ID != localId_)
		{
			...
			{
				std::wstring path = L"";
				userContext_->getDataBaseMgr()->getDiffTable()->deleteCompletePath(localId_, Key_LocalID, path);
			}
			else if(Diff_Running!=status)
			{
				userContext_->getDataBaseMgr()->getDiffTable()->updateErrorCode(Key_LocalID, localId_, errorCode);
			}
		}
		if(INVALID_ID != remoteId_)
		{
			...
			{
				std::wstring path = L"";
				userContext_->getDataBaseMgr()->getDiffTable()->deleteCompletePath(remoteId_, Key_RemoteID, path);
			}
			else if(Diff_Running!=status)
			{
				userContext_->getDataBaseMgr()->getDiffTable()->updateErrorCode(Key_RemoteID, remoteId_, errorCode);
			}
		}
	}
...
private:
	void getTopDiff(const OperNode& operNode)
	{
		if(Key_LocalID==operNode->keyType)
		{
			...
			userContext_->getDataBaseMgr()->getRelationTable()->getRemoteIdByLocalId(localId_, remoteId_);
		}
		else
		{
			remoteId_ = operNode->key;
			userContext_->getDataBaseMgr()->getRelationTable()->getLocalIdByRemoteId(remoteId_, localId_);
		}
		userContext_->getDataBaseMgr()->getDiffTable()->geOperList(localId_, Key_LocalID, localList_);
		userContext_->getDataBaseMgr()->getDiffTable()->geOperList(remoteId_, Key_RemoteID, remoteList_);
	}
	...
	void checkOper()
	{
		...
		{
			if(userContext_->getDataBaseMgr()->getLocalTable()->getPath(localId_).empty())
			{
		...
		*/
		if((INVALID_ID != remoteId_)&&(!remoteList_.empty()))
		{
			...
			if(RT_OK != userContext_->getDataBaseMgr()->getRemoteTable()->getPath(remoteId_, remotePath))
			{
				return;
			}
			...
			{
				if(userContext_->getDataBaseMgr()->getRemoteTable()->isNoSync(SD::Utility::FS::get_topdir_name(remotePath)))
				{
					deleteInvaildRemote();
				}
			}
	...
	void genSyncRuleKey()
	{
		...
		if(!userContext_->getConfigureMgr()->getSyncRules()->getActions(syncRuleKey_, actions_))
		{
	...
	void completeDiff()
	{
		...
		if(INVALID_ID != localId_)
		{
			userContext_->getDataBaseMgr()->getDiffTable()->deleteCompletePath(localId_, Key_LocalID, localPath);
		}

		std::wstring remotePath = L"";
		if(INVALID_ID != remoteId_)
		{
			userContext_->getDataBaseMgr()->getDiffTable()->deleteCompletePath(remoteId_, Key_RemoteID, remotePath);
		}
	...
	void deleteInvaildRemote()
	{
		...
		else
		{
			...
			userContext_->getDataBaseMgr()->getDiffTable()->completeDiff(Key_RemoteID, remoteId_);
			remoteList_.clear();
