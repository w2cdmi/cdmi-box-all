...
public:
	LocalDetectorImpl(UserContext* userContext)
		:userContext_(userContext)
		,changeJournal_(ChangeJournal::create(userContext_->getConfigureMgr()->getConfigure()->monitorRootPath()))
	...
	virtual int32_t start()
	{
		...
		int32_t ret = userContext_->getDataBaseMgr()->getLocalTable()->getRootNode(rootNode);
	...
	virtual int32_t fullDetect()
	{
		...
		if(!userContext_->getDataBaseMgr()->getRelationTable()->remoteIdIsExist(remoteRoot))
		{
			int32_t ret = RT_OK;
			...
			CHECK_RESULT(userContext_->getDataBaseMgr()->getRelationTable()->addRelation(remoteRoot,localRoot));
		}
	...
	virtual int32_t incDetect()
	{
		...
		if (0 > usn || 0 >= userContext_->getDataBaseMgr()->getLocalTable()->getCount())
		{
...
private:
	...
	int32_t fullDetectImpl()
	{
		...
		// full mode
		if (0 > usn || 
			0 >= userContext_->getDataBaseMgr()->getLocalTable()->getCount())
		{
			...
			if (0 > usn)
			{
				CHECK_RESULT(userContext_->getDataBaseMgr()->getLocalTable()->clearTable());
				CHECK_RESULT(userContext_->getDataBaseMgr()->getDiffTable()->clearDiff(Key_LocalID));
			}
			...
			// update autoInc
			iniHelper_.SetValue<int64_t>(L"", CONF_AUTOINC_ID, userContext_->getDataBaseMgr()->getDiffTable()->getMaxInc());
		}
		else
		{
			...
			CHECK_RESULT(userContext_->getDataBaseMgr()->getLocalTable()->initMarkStatus());
			...
			CHECK_RESULT(userContext_->getDataBaseMgr()->getLocalTable()->getNodesByMarkStatus(trashDeleteNodes, MS_Missed));
			OperNodes operNodes;
			...
			CHECK_RESULT(userContext_->getDataBaseMgr()->getDiffTable()->addOperList(operNodes));
			CHECK_RESULT(userContext_->getDataBaseMgr()->getLocalTable()->trashDeleteNodes(trashDeleteNodes));
		}
	...
	int32_t enumUsnAddAllImpl(LocalSyncNodes& syncNodes)
	{
		...
		LocalTable* localTable = userContext_->getDataBaseMgr()->getLocalTable();
		...
		std::wstring cachePath = userContext_->getConfigureMgr()->getConfigure()->cachePath();
		...
		CHECK_RESULT(userContext_->getDataBaseMgr()->getDiffTable()->addOperList(operNodes));
		CHECK_RESULT(localTable->addNodes(syncNodes));
		(void)userContext_->getDataBaseMgr()->getDiffTable()->insertIncPath(diffPathNodes);
	...
	int32_t enumUsnCompareAllImpl(LocalSyncNodes& syncNodes)
	{
		...
		IdList existList;
		int64_t rootId = changeJournal_->getRootId();
		std::wstring cachePath = Utility::FS::get_file_name(userContext_->getConfigureMgr()->getConfigure()->cachePath());
	...
	{
		return userContext_->getDataBaseMgr()->getLocalTable()->isExist(id);
	}
	...
	int32_t readUsnImpl(LocalSyncNodes& syncNodes)
	{
		...
		LocalSyncNode syncNode;
		OperNodes operNodes;
		param.localTable = userContext_->getDataBaseMgr()->getLocalTable();
		assert(NULL != param.localTable);
		param.diffTable = userContext_->getDataBaseMgr()->getDiffTable();
		assert(NULL != param.diffTable);

		int64_t rootId = changeJournal_->getRootId();
		std::wstring cacheName = Utility::FS::get_file_name(userContext_->getConfigureMgr()->getConfigure()->cachePath());
