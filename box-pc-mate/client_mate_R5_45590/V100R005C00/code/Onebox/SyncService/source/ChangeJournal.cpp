 ...
 std::auto_ptr<ChangeJournal> ChangeJournal::create(const std::wstring& root, bool async)
{
	 ...
	 ,isStop_(true)
 {
	 try
	 {
		 (void)memset_s(&ujd_, sizeof(USN_JOURNAL_DATA), 0, sizeof(USN_JOURNAL_DATA));
		 (void)memset_s(&overlapped_, sizeof(OVERLAPPED), 0, sizeof(OVERLAPPED));
...
int32_t ChangeJournalImpl::readUsnJournalAsync(const USN usn)
{
	...
	(void)memset_s(&rujd, sizeof(rujd), 0, sizeof(rujd));
	rujd.StartUsn = usn;
	...
	(void)memset_s(buf_.get(), UR_BUF_LEN, 0, UR_BUF_LEN);
	if(!DeviceIoControl(volumeHandle_, 
