...
#ifndef MODULE_NAME
#define MODULE_NAME ("RemoteDetector")
#endif
...
class RemoteDetectorImpl : public RemoteDetector
{
public:
	RemoteDetectorImpl(UserContext* userContext)
		:userContext_(userContext)
	...
	virtual ~RemoteDetectorImpl(void)
	{
	}
	...
	virtual int32_t incDetect()
	{
		...
		int64_t maxVersion = dataBaseMgr_->getSyncDataTable()->getMaxVersion();
		if(maxVersion==0)
		{
			return RT_OK;
		}
	...
	virtual int32_t fullDetect()
	{
		...
		if(fullDetectVersion_>0)
		{
			...
			if(dataBaseMgr_->getDiffTable()->getLeftDiffCount()>0)
			{
				SERVICE_DEBUG(MODULE_NAME, RT_OK, "the diff is being processed, skip fullDetect.");
				return RT_OK;
			}
		}
		boost::mutex::scoped_lock lock(GlobalMutex::remoteMutex());

		bool isAllCreate = false;
		//update remote tree exist flag 
		if(RT_CANCEL==dataBaseMgr_->getRemoteTable()->initMarkStatus())
		{
			isAllCreate = true;
		}
	...
	virtual int32_t dirDetecte(const int64_t& id)
	{
		...
		dataBaseMgr_->getDiffTable()->addOperList(remoteDiff);
		dataBaseMgr_->getRemoteTable()->replaceRemoteNodes(newRemoteNodes);
...
private:
	void getAllCreateDiff(RemoteNodes& syncData)
	{
		...
		CInIHelper iniHelper(userContext_->getConfigureMgr()->getConfigure()->userDataPath() + SYNC_CONF_NAME);
		dataBaseMgr_->getDiffTable()->addOperList(remoteDiff);
		dataBaseMgr_->getDiffTable()->insertIncPath(diffInfos);
		iniHelper.SetValue<int64_t>(L"", CONF_AUTOINC_ID, dataBaseMgr_->getDiffTable()->getMaxInc());

		dataBaseMgr_->getRemoteTable()->replaceRemoteNodes(newRemoteNodes);

		//update syncVersion
		SyncDataTable* syncDataTable = dataBaseMgr_->getSyncDataTable();
		if(NULL==syncDataTable)
		{
			return;
		}
		int64_t maxVersion = syncDataTable->getMaxVersion();
		iniHelper.SetValue<int64_t>(L"", CONF_SYNCVERSION_KEY, maxVersion);
	...
	void getRemoteDiff(RemoteNodes& syncData)
	{
			...
			it != syncData.end(); ++it)
		{
			...
			//set noSync
			if((ROOT_PARENTID==syncNode->parent)&&(RS_None ==syncNode->status))
			{
				if(isExsit&&isSync)
				{
					OperNode pOperNode(new st_OperNode());
		...
		dataBaseMgr_->getDiffTable()->addOperList(operList);
		dataBaseMgr_->getRemoteTable()->getChildren(deleteList);
		dataBaseMgr_->getRemoteTable()->trashDeleteNodes(deleteList);
		dataBaseMgr_->getRemoteTable()->replaceRemoteNodes(newRemoteNodes);
	}
		...
		DiffPathNodes& diffInfos)
	{
			...
			it != dirSyncData.end(); ++it)
		{
			...
			diffPathNode->localPath = SyncConfigure::getInstance(userContext_)->monitorRootPath() + diffPathNode->remotePath;
	...
	int32_t getDirMetadata(const int64_t& parentId, RemoteNodes& syncData)
	{
		...
		syncData.clear();
		ret = dataBaseMgr_->getSyncDataTable()->getLoginSyncData(syncData);
	...
	int32_t getChangeMetadata(int64_t curVersion, RemoteNodes& syncData)
	{
		...
		syncData.clear();
		CHECK_RESULT(dataBaseMgr_->getSyncDataTable()->getSyncData(syncData));
	...
	{
		IdList idList;
		dataBaseMgr_->getRemoteTable()->getNodesByMarkStatus(existList, idList, MS_Missed);
		...
		dataBaseMgr_->getRemoteTable()->trashDeleteNodes(idList);
		...
		dataBaseMgr_->getRemoteTable()->filterChildren(idList);
	...
	bool isSkipOper(const RemoteNode& remoteNode)
	{
		...
		//TODO
		//return dataBaseMgr_->getTransTaskTable()->isUploading(remoteNode->id, remoteNode->parent, remoteNode->name);
		return false;
	}
...
std::auto_ptr<RemoteDetector> RemoteDetector::create(UserContext* userContext)
{
	return std::auto_ptr<RemoteDetector>(new RemoteDetectorImpl(userContext));
}