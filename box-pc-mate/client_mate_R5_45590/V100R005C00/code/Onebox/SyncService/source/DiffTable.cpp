...
public:
		...
		:userContext_(userContext)
	{
		...
		needRefresh_ = true;
		rootLen_ = SyncConfigure::getInstance(userContext_)->monitorRootPath().length();
		lastLeftCnt_ = getLeftDiffCount();
	...
	{
		if((lastLeftCnt_>0)||(0==lastErrorCnt_))
		{
			return RT_OK;
		}
		//boost::mutex::scoped_lock lock(mutex_);
	...
	virtual int32_t getTopOper(OperNode& operNode)
	{
		boost::mutex::scoped_lock lock(mutex_);
		try
		{
	...
	virtual int32_t deleteCompletePath(const int64_t& key, const KeyType keyType, std::wstring& localPath)
	{
		...
		try
		{
			...
			CppSQLite3Buffer sql;
			sql.format("DELETE FROM %s WHERE %s = %d AND %s = %lld", 
								DIFFPATH_TABLE_NAME, 
								DIFFPATH_ROW_TYPE, (int)keyType,
								DIFFPATH_ROW_ID, key);
			int32_t iModify = db_.execDML(sql);
	...
	virtual int64_t getLeftDiffCount()
	{
		//boost::mutex::scoped_lock lock(mutex_);
		try
		{
			...
			if(!q1.eof())
			{
				lastLeftCnt_ = q1.getInt64Field(0);
			}
		}
		CATCH_SQLITE_EXCEPTION;
		return lastLeftCnt_;
	...
	virtual int64_t getErrorDiffCount()
	{
		try
		{
			CppSQLite3Buffer sql1;
			...
			if(!q1.eof())
			{
				lastErrorCnt_ = q1.getInt64Field(0);
			}
		}
		CATCH_SQLITE_EXCEPTION;
...
private:
	...
	bool needRefresh_;
	int32_t rootLen_;
	int64_t lastLeftCnt_;
	int64_t lastErrorCnt_;
};
