...
std::auto_ptr<OverlayIconMgr> OverlayIconMgr::create(UserContext* userContext)
{
	return std::auto_ptr<OverlayIconMgr>(new OverlayIconMgrImpl(userContext));
}
...
{
	if (path.length() <= SyncConfigure::getInstance(userContext_)->monitorRootPath().length()) 
	{
		return RT_OK;
	}
	{
	...
	return refreshOverlayIcon(Utility::FS::get_parent_path(path));
}
...
OverlayIconStatus OverlayIconMgrImpl::getOverlayIconStatus(const std::wstring& path)
{
	...
	if(dataBaseMgr_->getDiffTable()->isInDiff(path))
	{
		status = OS_Syncing;
	...
	ret = dataBaseMgr_->getLocalTable()->getNode(fileDirInfo.id, localNode);
	if (RT_OK != ret)
	{
		return status;
	}
	...
	if(LS_Normal==localNode->status)
	{
		...
		if(dataBaseMgr_->getDiffTable()->isInDiff(Key_LocalID, fileDirInfo.id))
		{
			...
			dataBaseMgr_->getDiffTable()->replaceIncPath(diffPathNodes);
		...
		ret = dataBaseMgr_->getRelationTable()->getRemoteIdByLocalId(fileDirInfo.id, remoteId);
		if(RT_OK == ret)
		{
			status = OS_Synced;
			addOverlayIconInfoToCache(localPath.path(), status);
