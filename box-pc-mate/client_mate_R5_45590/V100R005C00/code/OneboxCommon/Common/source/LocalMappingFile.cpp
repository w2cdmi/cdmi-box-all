...
LocalMappingFile::LocalMappingFile(const Path& path, UserContext* userContext)
	...
	,mappingHandle_(NULL)
{
	(void)memset_s(&si_, sizeof(SYSTEM_INFO), 0, sizeof(SYSTEM_INFO));
}
...
int32_t LocalMappingFile::open(const OpenMode mode)
{
	...
	DWORD shareFlag = (OpenRead == mode)?FILE_SHARE_READ|FILE_SHARE_WRITE:FILE_SHARE_READ;
...
int32_t LocalMappingFile::write(const unsigned char* buffer, const uint32_t size, const int64_t& offset)
{
	...
	memcpy_s((char*)fileMappingBuf+alignSize, (size_t)writeSize, buffer, (size_t)writeSize);
	...
	ret = GetLastError();
	(void)UnmapViewOfFile(fileMappingBuf);
	if (RT_OK != ret)
	{
