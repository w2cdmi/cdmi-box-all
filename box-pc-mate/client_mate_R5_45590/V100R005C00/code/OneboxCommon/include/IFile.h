...
struct FILE_VERSION_INFO
{
	...
	FILE_VERSION_INFO()
		...
		,modifiedBy(0L)
		,objectId(L"")
		,contentCreatedAt(INVALID_TIME)
		,contentModifiedAt(INVALID_TIME){}
};

typedef std::vector<FILE_VERSION_INFO> LIST_FILEVERSION_RESULT;
...
enum PROPERTY_MASK
{
	...
	PROPERTY_MODIFYID = 0x00400,
	PROPERTY_EXTRATYPE = 0x00800
};
...
enum FOLDER_EXTRA_TYPE
{
	FOLDER_EXTRA_TYPE_NONE = 0,
	FOLDER_EXTRA_TYPE_COMPUTER,
	FOLDER_EXTRA_TYPE_DISK
};
...
public:
	...
	virtual FILE_DIR_INFO property(const PROPERTY_MASK& mask = PROPERTY_MASK_ALL)
	{
		...
		if (PROPERTY_SIGNATURE&mask)
			property.fingerprint = property_.fingerprint;
		if (PROPERTY_FLAGS&mask)
			property.flags = property_.flags;
		if (PROPERTY_MODIFYID&mask)
			property.modifiedId = property_.modifiedId;
		return property;
	}
	...
	virtual void property(const FILE_DIR_INFO& property, const PROPERTY_MASK& mask = PROPERTY_MASK_ALL)
	{
		...
		if (PROPERTY_SIGNATURE&mask)
			property_.fingerprint = property.fingerprint;
		if (PROPERTY_FLAGS&mask)
			property_.flags = property.flags;
		if (PROPERTY_MODIFYID&mask)
			property_.modifiedId = property.modifiedId;
	}
	...
	virtual int32_t copy(const Path& newParent, bool autoRename) {return RT_NOT_IMPLEMENT;}

	virtual int32_t move(const Path& newParent, bool autoRename) {return RT_NOT_IMPLEMENT;}
