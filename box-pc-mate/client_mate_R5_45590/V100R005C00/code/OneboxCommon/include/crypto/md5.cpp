...
/* Default construct. */
CMD5::CMD5() {
	(void)memset_s(_digest, sizeof(byte), 0, sizeof(byte));
	(void)memset_s(_buffer, sizeof(_buffer), 0, sizeof(_buffer));
    reset();
}
...
void CMD5::update(const byte* input, size_t length) {
    ...
    if (length >= partLen) {
        ...
        memcpy_s(&_buffer[index], partLen, input, partLen);
        transform(_buffer);
    ...
    memcpy_s(&_buffer[index], length - i, &input[i], length - i);
}
...
void CMD5::final() {
    ...
    memcpy_s(oldState, 16, _state, 16);
    memcpy_s(oldCount, 8, _count, 8);
    ...
    memcpy_s(_state, 16, oldState, 16);
    memcpy_s(_count, 8, oldCount, 8);
}
...
void CMD5::transform(const byte block[64]) {
	...
	(void)memset_s(&x, sizeof(uint32) * 16, 0x00, sizeof(uint32) * 16);
