...
template<typename DataUnit>
int32_t JsonParser::Parser<DataUnit>::parseLeaf(const std::string& strKey, Json::Value& jsonLeafValue)
{
	if(jsonRoot_.isNull()||!jsonRoot_[strKey].isObject())
	{
		jsonLeafValue = Json::nullValue;
		return RT_OK;
	}

	jsonLeafValue = jsonRoot_[strKey];
