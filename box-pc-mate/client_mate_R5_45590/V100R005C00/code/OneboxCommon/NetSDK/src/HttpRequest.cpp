...
#define curl_easy_setopt_safe(opt, val)        \
    ret = curl_easy_setopt(curl_, opt, val);   \
	if (ret != CURLE_OK)                       \
    {                                          \
        return ret;                            \
    }
...
void HttpRequest::setUri(const std::string &strEndpoint, const std::string  &strUri)
{
...
int32_t HttpRequest::setupCurl(RequestParam &param, Configure &config)
{
	...
	if (config.useSSL())
	{
		...
		curl_easy_setopt_safe(CURLOPT_CAINFO, Utility::String::wstring_to_string(config.capath()).c_str());
	}	
    ...
    else if (HttpRequestTypePOST == httpMethod_)
    {
		...
		{
			curl_easy_setopt_safe(CURLOPT_COPYPOSTFIELDS, param.readData->pBuf);
			curl_easy_setopt_safe(CURLOPT_POSTFIELDSIZE_LARGE, param.readData->lBufLen);
		}
    }
	...
	{
		curl_easy_setopt_safe(CURLOPT_XFERINFOFUNCTION, param.progressCallback);
		curl_easy_setopt_safe(CURLOPT_XFERINFODATA, param.progressData);
    ...
    if (config.requestConnectTimeout())
    {
        curl_easy_setopt_safe(CURLOPT_CONNECTTIMEOUT, config.requestConnectTimeout());
    }
	if (config.requestTimeout())
	{
		curl_easy_setopt_safe(CURLOPT_LOW_SPEED_LIMIT, config.requestLowSpeedLimit());
		curl_easy_setopt_safe(CURLOPT_LOW_SPEED_TIME, config.requestTimeout());
	}
    ...
    // do not receive header in WRITEFUNCTION
                      ...
                      void *data)
{
    ...
    (void)memset_s(pcBuf, len + 1, 0, len + 1);
    memcpy_s(pcBuf, len, ptr, len);
