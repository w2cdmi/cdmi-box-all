...
public:
	...
	//拖拽相关 2015.3.6
	virtual HRESULT OnDragEnter( CDataObjectEx *pDataObj, DWORD grfKeyState, POINTL ptl,DWORD *pdwEffect);  
	virtual HRESULT OnDragOver(DWORD grfKeyState, POINTL pt,DWORD *pdwEffect);  
	virtual HRESULT OnDragLeave();  
	virtual HRESULT OnDrop(CDataObjectEx *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect);
	void SetDropEnable(bool bDrop);
	bool IsDropEnable();

	//设置窗口阴影 2015.10.21
	void SetShadowEnable(bool bShadowEnable);
	void SetShadowSize(int size);
	void SetShadowColor(COLORREF shadowColor);
	void SetShadowSharpness(UINT sharpness);
	void SetShadowDarkness(UINT darkness);
	void SetShadowPos(POINT shadowPos);
	bool SetShadow();
	CWndShadow m_hWndShadow;
