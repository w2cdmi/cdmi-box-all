...
CControlUI::~CControlUI()
{
	...
	if ( NULL != m_pNotifyIcon ) delete m_pNotifyIcon;
}
...
CDuiString CControlUI::GetEndellipsisText(LPCTSTR pstrText, int maxWidth, int font) const
{
	HDC hDC = m_pManager->GetPaintDC();
	TEXTMETRIC* pTm = &m_pManager->GetDefaultFontInfo()->tm;
	TFontInfo* pFontInfo = m_pManager->GetFontInfo(font);	//字体大小
	...
	while( *p != _T('\0') && *p != _T('\n') ) {
		...
		{
			endellipsisText += L"...";
			return endellipsisText;
		}
		if(szText.cx <= (maxWidth-12))
		{
			endellipsisText += *p;
		}
		p = ::CharNext(p);
...
int CControlUI::GetTextWidth(LPCTSTR pstrText, int font) const
{
	HDC hDC = m_pManager->GetPaintDC();
	TEXTMETRIC* pTm = &m_pManager->GetDefaultFontInfo()->tm;
	TFontInfo* pFontInfo = m_pManager->GetFontInfo(font);	//字体大小
...
void CControlUI::SetContextMenu(CContextMenuUI* pMenu)
{
	m_pMenu = pMenu;
}

CContextMenuUI* CControlUI::GetContextMenu()
{
	return m_pMenu;
}
...
void CControlUI::SetDropEnabled(bool bDrop)
{
	m_bDragEnabled = bDrop;
}
    ...
    if( event.Type == UIEVENT_CONTEXTMENU )
    {
        if( IsContextMenuUsed() ) {
			if (!m_sMenuXmlFile.IsEmpty()) {
				m_pMenu = new CContextMenuUI();
				if ( NULL == m_pMenu ) return;
				m_pMenu->SetXmlFile(m_sMenuXmlFile);
			}
			m_pManager->SendNotify(this, DUI_MSGTYPE_MENU, event.wParam, event.lParam);
			if (m_pMenu) m_pMenu->SetManager(GetManager(), this, true);
            return;
        }
    }
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("droptarget")) == 0 ) SetDropEnabled(_tcscmp(pstrValue, _T("true")) == 0);  //2015.3.6
