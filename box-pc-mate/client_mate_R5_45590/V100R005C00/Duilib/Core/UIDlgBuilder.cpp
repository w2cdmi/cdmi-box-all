            ...
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
                    ...
                    if( _tcscmp(pstrName, _T("size")) == 0 ) {
						...
						pManager->SetBorderSize(_ttoi(pstrValue));
					} 
					else if( _tcscmp(pstrName, _T("bordercolor")) == 0 ) {
						if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
						LPTSTR pstr = NULL;
						DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
						pManager->SetBorderColor(clrColor);
					} 
					...
					else if( _tcscmp(pstrName, _T("shadowenable")) == 0 ) {
						pManager->SetShadowEnable(_tcscmp(pstrValue, _T("true")) == 0);
					}  
					else if( _tcscmp(pstrName, _T("shadowsize")) == 0 ) {
						LPTSTR pstr = NULL;
						int shadowsize = _tcstol(pstrValue, &pstr, 10);
						pManager->SetShadowSize(shadowsize);
					}
					else if( _tcscmp(pstrName, _T("shadowcolor")) == 0 ) {
						if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
						LPTSTR pstr = NULL;
						COLORREF clrColor = _tcstoul(pstrValue, &pstr, 16);
						pManager->SetShadowColor(clrColor);
					}
					else if( _tcscmp(pstrName, _T("sharpness")) == 0 ) {
						LPTSTR pstr = NULL;
						UINT sharpness = _tcstoul(pstrValue, &pstr, 10);
						pManager->SetShadowSharpness(sharpness);
					}
					else if( _tcscmp(pstrName, _T("darkness")) == 0 ) {
						LPTSTR pstr = NULL;
						UINT darkness = _tcstoul(pstrValue, &pstr, 10);
						pManager->SetShadowDarkness(darkness);
					} 
					else if( _tcscmp(pstrName, _T("shadowpos")) == 0 ) {
						LPTSTR pstr = NULL;
						POINT shadowpos;
						shadowpos.x = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
						shadowpos.y = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
						pManager->SetShadowPos(shadowpos);
					}  
				}
            }
        }
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 8:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_MENUITEM) == 0 )          pControl = new CMenuItemUI;
                break;
            case 9:
        ...
        if( node.HasAttributes() ) {
			...
			// Use language file to init 'text' and 'tooltip' attributs
			if (NULL != m_pCallback) {
				m_pCallback->InitLanguage(pControl);
			}
        }
        if( pManager ) {
