...
namespace DuiLib
{
	...
	bool CContainerUI::IsDropEnabled()
	{
		return CControlUI::IsDropEnabled();
	}
	...
	void CContainerUI::SetScrollPos(SIZE szPos)
	{
		...
		if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) {
			...
			if(cy>0)
			{
				m_pManager->SendNotify(this, DUI_MSGTYPE_NEXTPAGE, m_pVerticalScrollBar->GetScrollRange(), m_pVerticalScrollBar->GetScrollPos());
			}
			else if(cy<0)
			{
				m_pManager->SendNotify(this, DUI_MSGTYPE_LASTPAGE, m_pVerticalScrollBar->GetScrollRange(), m_pVerticalScrollBar->GetScrollPos());
			}
		}
	...
	CControlUI* CContainerUI::FindSubControlsByClass(LPCTSTR pstrClass, int iIndex)
	{
		CControlUI* pSubControl=NULL;
		CStdPtrArray* pSubControlArray=NULL;
		pSubControlArray = GetManager()->FindSubControlsByClass(this, pstrClass);
		pSubControl=static_cast<CControlUI*>(pSubControlArray->GetAt(iIndex));
		return pSubControl;
	}
