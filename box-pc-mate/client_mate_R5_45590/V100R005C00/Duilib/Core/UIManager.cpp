...
m_uSharpness(5),
m_uDarkness(50),
m_cShadowColor(0xFF000000)
,m_nBorderSize(1)
,m_dwBorderColor(4290493371)
{
...
CPaintManagerUI::~CPaintManagerUI()
{
	...
	{
		m_pDropTarget->Release(); //拖拽 2015.3.6
		m_pDropTarget->DragDropRevoke(GetPaintWindow());  //拖拽 2015.3.6
	}
}
...
void CPaintManagerUI::SetBorderSize(int nSize)
{
	m_nBorderSize = nSize;
}

int CPaintManagerUI::GetBorderSize() const
{
	return m_nBorderSize;
}

void CPaintManagerUI::SetBorderColor(DWORD dwBorderColor)
{
	m_dwBorderColor = dwBorderColor;
}

DWORD CPaintManagerUI::GetBorderColor() const
{
    ...
    case WM_MOUSEMOVE:
        {
            ...
            POINT pt = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
			if (m_ptLastMousePos.x == pt.x && m_ptLastMousePos.y == pt.y) break;
            m_ptLastMousePos = pt;
...
HRESULT CPaintManagerUI::OnDragEnter( CDataObjectEx *pDataObj, DWORD grfKeyState, POINTL ptl,  DWORD *pdwEffect)  
{  
...
HRESULT  CPaintManagerUI::OnDrop(__RPC__in_opt CDataObjectEx *pDataObj, DWORD grfKeyState, POINTL ptl, __RPC__inout DWORD *pdwEffect)  
{  
	POINT pt={ptl.x,ptl.y};  
	::ScreenToClient(m_hWndPaint,&pt);  
	if( m_pEventDrop != NULL ) 
	{  
		...
		//FORMATETC etc={CF_TEXT,0,DVASPECT_CONTENT,-1,TYMED_HGLOBAL};
		IEnumFORMATETC *etc;
		FORMATETC etc2;
		STGMEDIUM stg;
		...
		while (res == S_OK)
		{
			...
			if(res == S_OK)
			{
				if (etc2.cfFormat == CF_HDROP)
				{
					...
					PostMessage(m_hWndPaint,WM_DROPFILES,(WPARAM)stg.hGlobal,0);
				}
			}
		}		
	}
...
bool CPaintManagerUI::IsDropEnable()
{
...
void CPaintManagerUI::SetDropEnable(bool bDrop)
{
	...
	if (IsDropEnable())
	{
		m_pDropTarget = new CDropTargetEx(true); //拖拽 2015.3.6
		//m_pDropTarget->setEnableWnd(true);
		if (m_pDropTarget != NULL)
		{
			m_pDropTarget->DragDropRegister(this,GetPaintWindow());  //拖拽 2015.3.6  
		}
	}
}

void CPaintManagerUI::SetShadowEnable(bool bShadowEnable)
{
	m_bShadowEnable = bShadowEnable;
}

void CPaintManagerUI::SetShadowSize(int size)
{
	m_iShadowSize = size;
}

void CPaintManagerUI::SetShadowColor(COLORREF shadowColor)
{
	m_cShadowColor = shadowColor;
}

void CPaintManagerUI::SetShadowSharpness(UINT sharpness)
{
	m_uSharpness = sharpness;
}

void CPaintManagerUI::SetShadowDarkness(UINT darkness)
{
	m_uDarkness = darkness;
}

void CPaintManagerUI::SetShadowPos(POINT shadowPos)
{
	m_pShadowPos = shadowPos;
}

bool CPaintManagerUI::SetShadow()
{
	if(!m_hWndPaint)
	{
		return false;
	...
	if (m_bShadowEnable)
	{
		m_hWndShadow.Initialize(CPaintManagerUI::GetInstance());
		LONG styleValue = ::GetWindowLong(m_hWndPaint, GWL_STYLE);
		::SetWindowLong(m_hWndPaint, GWL_STYLE, styleValue | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);
		m_hWndShadow.Create(m_hWndPaint);
		m_hWndShadow.SetSize(m_iShadowSize);
		m_hWndShadow.SetColor(m_cShadowColor);
		m_hWndShadow.SetSharpness(m_uSharpness);
		m_hWndShadow.SetDarkness(m_uDarkness);
		m_hWndShadow.SetShadowPos(m_pShadowPos.x,m_pShadowPos.y);
	}
	return true;
}} // namespace DuiLib
