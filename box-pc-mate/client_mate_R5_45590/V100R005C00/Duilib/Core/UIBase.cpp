...
void CWindowWnd::CenterWindow(RECT parentRect)
{
	ASSERT(::IsWindow(m_hWnd));
	...
	if( hWndCenter == NULL )
		...
		if (((parentRect.top <0)&&(parentRect.bottom<0)) || ((parentRect.left<0)&&(parentRect.right<0)))
			parentRect = rcArea;
	}
	...
	int xLeft = (parentRect.right - parentRect.left) / 2 + parentRect.left - DlgWidth / 2  + rcCenter.left;
	int yTop = (parentRect.bottom - parentRect.top) / 2 + parentRect.top - DlgHeight / 2 + rcCenter.top;
...
void CWindowWnd::CenterWindow()
{
	...
	if( hWndCenter == NULL )
		...
		::GetWindowRect(hWndCenter, &rcCenter);	
		if (((rcCenter.top <0)&&(rcCenter.bottom<0)) || ((rcCenter.left<0)&&(rcCenter.right<0)))
			rcCenter = rcArea;
	}
