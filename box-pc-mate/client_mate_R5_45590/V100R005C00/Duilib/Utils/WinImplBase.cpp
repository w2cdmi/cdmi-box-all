...
bool WindowImplBase::InitLanguage(CControlUI* control)
{
	return false;
}
...
{
	if ( (UINT)wParam == PAINT_BORDER_TIMER ) {
		...
		//::KillTimer(*this, PAINT_BORDER_TIMER);
		bHandled = TRUE;
		return 0;
	}
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (m_PaintManager.GetBorderSize() != 0) {
		rcClient.right += m_PaintManager.GetBorderSize();
		rcClient.bottom += m_PaintManager.GetBorderSize();
	}
