...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		{
			static bool bSendInvalid = false;
			wchar_t temp = wParam;
			if ( (NULL != m_pOwner) 
				&& (NULL != m_pOwner->m_pManager) 
				&& (NULL != m_pOwner->m_pLimitChar) 
				&& (NULL != StrChr(m_pOwner->m_pLimitChar, temp))
				&& (temp != L' '))
			{
				bSendInvalid = true;
				m_pOwner->m_pManager->SendNotify(m_pOwner, DUI_MSGTYPE_EDIT_INVALID_CHARACTER, wParam, lParam);
				return S_FALSE;
			}
			else
			{
				if ( bSendInvalid && (NULL != m_pOwner) 
					&&(NULL != m_pOwner->m_pManager) )
				{
					m_pOwner->m_pManager->SendNotify(m_pOwner, DUI_MSGTYPE_EDIT_RESTOR_NORMAL, wParam, lParam);
				}
			}
			bHandled = FALSE;
		}
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0),m_pLimitChar(NULL)
	{
