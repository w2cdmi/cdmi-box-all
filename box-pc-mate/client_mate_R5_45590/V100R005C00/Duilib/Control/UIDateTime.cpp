...
#define DT_KEEP   3
	...
	public:
		...
		void SetLanguageType(bool bChinese = true);
	protected:
	...
	CDateTimeWnd::CDateTimeWnd() : m_pOwner(NULL), m_hBkBrush(NULL), m_bInit(false), m_bChinese(true)
	{
	}

	void CDateTimeWnd::SetLanguageType(bool bChinese /* = true */)
	{
		m_bChinese = bChinese;
	}
	...
	void CDateTimeWnd::Init(CDateTimeUI* pOwner)
	{
		...
		LRESULT style = ::SendMessage(m_hWnd, DTM_GETMCSTYLE, 0, 0);
		style |= MCS_NOTODAY;
		::SendMessage(m_hWnd, DTM_SETMCSTYLE, 0, style);

		CDuiString format = L"";
		if (m_bChinese)
			format = L"yyyy'-'MM'-'dd' 'HH':'mm";
		else
			format = L"MM'/'dd'/'yyyy' 'HH':'mm";
		::SendMessage(m_hWnd, DTM_SETFORMAT, 0, (LPARAM)format.GetData());
	...
	void CDateTimeUI::UpdateText()
	{
		...
		else if (m_nDTUpdateFlag == DT_UPDATE)
		{
			CDuiString sText;
			if (m_bChinese)
				sText.SmallFormat(_T("%4d-%02d-%02d %02d:%02d"),
				m_sysTime.wYear, m_sysTime.wMonth, m_sysTime.wDay, m_sysTime.wHour, m_sysTime.wMinute);
			else
				sText.SmallFormat(_T("%02d/%02d/%4d %02d:%02d"),
				m_sysTime.wMonth, m_sysTime.wDay, m_sysTime.wYear, m_sysTime.wHour, m_sysTime.wMinute);
			SetText(sText);
		}
	}
	
	void CDateTimeUI::SetLanguageType(bool bChinese)
	{
		m_bChinese = bChinese;
		m_nDTUpdateFlag = DT_UPDATE;
		UpdateText();
	...
	void CDateTimeUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_SETFOCUS && IsEnabled() ) 
		{
			...
			ASSERT(m_pWindow);
			m_pWindow->SetLanguageType(m_bChinese);
			m_pWindow->Init(this);
