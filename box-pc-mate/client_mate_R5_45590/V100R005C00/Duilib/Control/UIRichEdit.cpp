...
void CTxtWinHost::TxViewChange(BOOL fUpdate) 
{
	...
	{
		m_re->Invalidate();
		//m_re->GetManager()->SendNotify(m_re,DUI_MSGTYPE_TEXTCHANGED);
	}
}
...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
    m_bWantCtrlReturn(true), m_bRich(false), m_bReadOnly(false), m_bWordWrap(false), m_dwTextColor(0), m_iFont(-1), 
...
void CRichEditUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_SCROLLWHEEL ) {
		...
		else if(event.wParam == SB_LINEDOWN)
		{
			LineDown();
		}
		return;
