...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1), m_selects(sizeof(int))
{
...
double CListUI::CalculateDelay(double state)
{
	return pow(state, 2);
}
...
void CListUI::DoEvent(TEventUI& event)
{
	...
	case  UIEVENT_TIMER: 
		{
			if (event.wParam == DUI_TIMERID::SCROLL_TIMERID)
			{
				if (m_iDelayLeft > 0)
				{
					--m_iDelayLeft;
				...
				m_iDelayNumber = 0;
				m_iDelayLeft = 0;
				m_pManager->KillTimer(this, DUI_TIMERID::SCROLL_TIMERID);
				return;
			}
		}			
		break;
    case UIEVENT_SCROLLWHEEL:
        {
			...
			m_pManager->SetTimer(this, DUI_TIMERID::SCROLL_TIMERID, 50U);
			return;
        }
        break;
	case UIEVENT_BUTTONDOWN:
		{
			...
			if ( MOUSE_MOVE_TYPE_INVALID == GetMouseMoveType() && IsEncloseSelect() )
			{
				...
				m_pScrollDownPoint = m_pList->GetScrollPos();
	...
	case UIEVENT_MOUSEMOVE:
		{
			if ( MOUSE_MOVE_TYPE_SELECT == GetMouseMoveType() && IsEncloseSelect() )
			{
				ShowSelectWnd(m_pButtonDownPoint, event.ptMouse, true);

				POINT pt = m_pButtonDownPoint;
				pt.y = pt.y - ( GetScrollPos().cy - m_pScrollDownPoint.cy );
...
bool CListUI::SelectRange(POINT ptstart, POINT ptend)
{
	int iCount = GetCount();
	RECT rctemp;
	m_selects.Empty();
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if ( _tcscmp(pstrName, _T("encloseselect")) == 0 )SetEncloseSelect(_tcscmp(pstrValue, _T("true")) == 0);
...
void CListUI::SetMouseMoveType(int type)
{
	m_iMouseMoveType = type;
}

int CListUI::GetMouseMoveType( )
{
	return m_iMouseMoveType;
}

void CListUI::SetButtonDownPoint( POINT pt )
{
	m_pButtonDownPoint = pt;
	m_pScrollDownPoint = m_pList->GetScrollPos();
}

void CListUI::SetButtonUpPoint( POINT pt )
{
	m_pButtonUpPoint = pt;
}

POINT CListUI::GetButtonDownPoint()
{
	return m_pButtonDownPoint;
}

POINT CListUI::GetButtonUpPoint()
{
	return m_pButtonUpPoint;
}

void CListUI::ShowSelectWnd( POINT posstart, POINT posend, bool isShow)
{
	if (m_wSelectWnd)
	{
		...
		if (isShow)
		{
			...
			if ( this->m_pList->GetVerticalScrollBar() )
			{
				iVScrollWidth = this->m_pList->GetVerticalScrollBar()->GetWidth();
			}
			if (this->m_pList->GetHorizontalScrollBar())
			{
				iHScrollHeight = this->m_pList->GetHorizontalScrollBar()->GetHeight();
			}

			posstart.y = posstart.y - (GetScrollPos().cy - m_pScrollDownPoint.cy);
			if (posstart.y > this->m_pList->GetPos().bottom )
			{
				posstart.y = this->m_pList->GetPos().bottom;
			}
			else if( posstart.y < this->m_pList->GetPos().top )
			{
				posstart.y = this->m_pList->GetPos().top;
			}

			if ( posend.x > this->m_pList->GetPos().right - iVScrollWidth )
			{
				posend.x = this->m_pList->GetPos().right - iVScrollWidth;
			}
			else if( posend.x < this->m_pList->GetPos().left )
			{
				posend.x = this->m_pList->GetPos().left;
			}

			if (posend.y > this->m_pList->GetPos().bottom - iHScrollHeight )
			{
				posend.y = this->m_pList->GetPos().bottom - iHScrollHeight;
			}
			else if( posend.y < this->m_pList->GetPos().top )
			{
				posend.y = this->m_pList->GetPos().top;
			}

			POINT pttemp = posstart;
			int height = 0;
			int width = 0;
			POINT ptwindowpos;
			ptwindowpos.x = -1;
			...
			if (posendtemp.y > this->m_pList->GetPos().bottom )
			{
			...
			else if( posendtemp.y < this->m_pList->GetPos().top)
			{
...
void CListUI::SetSelectWnd( CWindowWnd* window, HWND parenthwnd, CPaintManagerUI* paintmanager )
{
	m_wSelectWnd = window;
...
void CListUI::setCurEnter( int curEnter )
{
	m_iCurEnter = curEnter;
}

int CListUI::getCurEnter()
{
	return m_iCurEnter;
}

bool CListUI::IsEncloseSelect()
{
	return m_bEncloseSelect;
}

void CListUI::SetEncloseSelect( bool bEncloseSelect )
{
	m_bEncloseSelect = bEncloseSelect;
}

SIZE CListUI::GetScrollDownPoint()
{
	return m_pScrollDownPoint;
}

CStdValArray* CListUI::GetSelects()
{
	return &m_selects;
}
...
void CListBodyUI::SetScrollPos(SIZE szPos)
{
    ...
    if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) {
		...
		if(cy>0)
		{
			m_pManager->SendNotify(this->GetParent(), DUI_MSGTYPE_NEXTPAGE, m_pVerticalScrollBar->GetScrollRange(), m_pVerticalScrollBar->GetScrollPos());
		}
		else if(cy<0)
		{
			m_pManager->SendNotify(this->GetParent(), DUI_MSGTYPE_LASTPAGE, m_pVerticalScrollBar->GetScrollRange(), m_pVerticalScrollBar->GetScrollPos());
		}
    }
...
void CListBodyUI::SetPos(RECT rc)
{
    ...
    /*if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) 
        szAvailable.cx += m_pVerticalScrollBar->GetScrollRange();*/
...
bool CListContainerElementUI::Select(bool bSelect,bool bCallback)
{
    ...
    if( !IsEnabled() ) return false;
    //if( bSelect == m_bSelected ) return true;
	m_bSelected = bCtrl?(!m_bSelected):bSelect;
	if (m_bSelected) {
		m_uButtonState |= UISTATE_HOT;
	}
	else {
