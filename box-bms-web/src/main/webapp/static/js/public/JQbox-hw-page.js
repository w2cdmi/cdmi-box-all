...
(function() {
	...
	var _createPage = function(target, opts){
		...
		var pageSkip = '<div id="'+ _idMap.pageSkipId +'" class="page-skip pull-right form-search"><span class="page-skip-text">'+ _text.pageSkip +'</span>'+
						'<div class="input-append"><input id="'+ _idMap.pageSkipTextId +'" class="search-query span1" type="text" onkeyup="this.value=this.value.replace(/[^0-9]*$/,\'\')" ondragenter="return false" onpaste="return !clipboardData.getData(\'text\').match(/\D/)" /><button class="btn" id="'+ _idMap.pageSkipBtnId +'" type="button"><i class="icon-chevron-right"></i></button></div>'+
						'</div>';
	...
	var _createPageBody = function(target, opts, _idMap, curPage, perDis, totaldata){
		if(totaldata != null){
			if(totaldata == 0 || (opts.showPageSet == false && totaldata <= perDis)){
		...
		if(opts.showPageSet == true){
			...
			$("#"+_idMap.pageItemSelectId).bind("change",function(){
		...
		if(totalPages > 1){
			...
			if(curPage <= readyNum + 2){
				minNum = 2;
			}else if(curPage >= totalPages - readyNum){
				minNum = totalPages - readyNum*2;
				if(minNum < 2){
					minNum = 2;
				}
			}else{
				minNum = curPage-readyNum;
			}
			
			if(curPage >= totalPages - readyNum - 1){
				maxNum = totalPages-1;
			}else if(curPage <= readyNum + 1){
				maxNum = readyNum*2 + 1;
				if(maxNum > totalPages-1){
					maxNum = totalPages-1;
				}
			}else{
				maxNum = curPage+readyNum;
			}
			...
			//完成页码整体的填充
			var pageMainBody = '<button type="button" class="btn btn-small" id="'+ _idMap.pageFirstId +'">1</button>' +
								dynamicPages +
	...
	$.fn.setPageData = function(opts, curPage, perDis, totaldata){
		...
		if(parent.get(0) != null){
			...
			if(opts.showPageSet == true){
				$("#"+ _idMap.pageItemSelectId).unbind(); //清除绑定事件
			}
