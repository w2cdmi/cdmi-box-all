...
<body>
<form action="${ctx}/app/adminlog/log/list" method="post" id="searchForm" name="searchForm"> 
		...
		<div class="form-left" style="width:42%">
	        <div class="control-group">
        		...
        		<c:forEach items="${operateTypeList}" var="oper">
        			<option value="<c:out value='${oper.operateType.code}'/>" <c:if test="${queryCondition.operateType == oper.operateType.code}">selected="selected"</c:if>><c:out value='${oper.operatrDetails}'/></option>
        		</c:forEach>
			</select>
	            </div>
	        </div>
        ...
        <table class="table table-bordered table-striped">
          ...
          <c:forEach items="${adminLogList.content}" var="adminLog">
            <tr>
                ...
                <td title="${cse:htmlEscape(adminLog.operateType)}"><c:out value='${adminLog.operateType}'/></td>
