...
<body>
	<div class="sys-content sys-content-stati">
		<h5><spring:message code="statistics.historyUserAccount" /></h5>
		<div class="form-horizontal form-con clearfix">
			<form name="createHistUserForm" id="createHistUserForm" class="row-fluid">
				<div class="span4">
					...
					<div class="control-group">
						...
						<div class="controls">
							<select class="span12" id="statistics_field" name="statistics_field">
				...
				<div class="span8">
					<div class="control-group">
						...
						<div class="controls">
							<input type="hidden" id="statisticsField" name="statisticsField"/>
		...
		<div class="chart-con" id="histStatisChartContent" style="display:none;">
				<img id="histStatisChart" name="histStatisChart"/>
		</div>

		<div class="table-con" id="appHistStatisTable" style="display: none"></div>
		<div class="table-con" id="regionHistStatisTable" style="display: none"></div>
...
var headData = {
		...
		"userCount" : {
			"title" : '<spring:message code="statistics.userAccount" />',
			"width" : "25%"
		},
		"unit" : {
...
function init() {
	opts_viewGrid = $("#appHistStatisTable").comboTableGrid({
		headData : headData,
		ItemOp : "user-defined",
		dataId : "id",
	...
	region_Opts_viewGrid = $("#regionHistStatisTable").comboTableGrid({
...
$(document).ready(function(){
	init();
	if($("#statistics_field").val() == "app"){
		$("#statisticsField").val("app");
		$("#regionSelect").hide();
	}else{
		$("#statisticsField").val("region");
		$("#appSelect").hide();
	}
	$("#statistics_field").change(function(){
		if($(this).val() == "app"){
			$("#statisticsField").val("app");
			$("#regionSelect").hide();
			$("#appSelect").show();
...
function submitCondition() {
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/statistics/histUserAccount",
		data :  $("#createHistUserForm").serialize(),
		...
		success : function(data) {
			...
			var histStatisChart = "${ctx}/"+ data.urls[0];
			...
			if(histStatisChart == "${ctx}/"){
				...
				if (catalogData == null){
					$("#appHistStatisTable").hide();
					$("#regionHistStatisTable").hide();
					return;
				}
				if(catalogData[0].legend == 'All Applications' || catalogData[0].legend == $("#appId").val()){
					$("#regionHistStatisTable").hide();
					$("#appHistStatisTable").show();
					$("#appHistStatisTable").setTableGridData(catalogData, opts_viewGrid);
				}else{
					$("#appHistStatisTable").hide();
					$("#regionHistStatisTable").show();
					$("#regionHistStatisTable").setTableGridData(catalogData, region_Opts_viewGrid);
				}
			}
