...
<body>
	<div class="sys-content sys-content-stati">
		<h5><spring:message code="system.storage.history"/></h5>
		<div class="form-horizontal form-con clearfix">
			<form id="systemStoreForm" name="systemStoreForm" class="row-fluid">
				<div class="span4">
		...
		<div class="table-con" id="systemStoreTable" style="display: none"></div>
	</div>
</body>
</html>
...
var headData = {
		"actualSpaceCount":{
			"title":"<spring:message code='distence.storage'/>(MB)",
			"width":"30%"
		},
		"spaceCount":{
			"title":"<spring:message code='dis.storage'/>(MB)",
			"width":"30%"
		},
		"unit":{
	...
	function initTable()
	{
		opts_viewGrid = $("#systemStoreTable").comboTableGrid({
			headData : headData,
			ItemOp : "user-defined",
			dataId : "date",
	...
	function createView() {
		...
		$.ajax({
			type : "POST",
			url : "${ctx}/statistics/getSystemStoreHistoryView",
			data : $("#systemStoreForm").serialize(),
			error : function(data) {
			...
			success : function(data) {
				...
				if (histStatisChart == "${ctx}/") {
					...
					$("#systemStoreTable").show();
					$("#systemStoreTable").setTableGridData(catalogData, opts_viewGrid);
				}
