...
<body>
	<div class="sys-content sys-content-stati">
		<h5><spring:message code="statistics.filestore.add.num"/></h5>
		<div class="form-horizontal form-con clearfix">
			<form name="addHistoryFileStoreForm" id="addHistoryFileStoreForm" class="row-fluid">
				<div class="span4">
				...
				<div class="span8">
					<div class="control-group">
						...
						<div class="controls">
							...
							<input  class= "btn btn-primary" type="button" onclick="createView()" value ='<spring:message code="statistics.createChart" />'/>
		...
		<div class="table-con" id="fileStoreAddTable" style="display: none"></div>
	</div>
</body>
</html>
...
var headData = {
		"spaceUsed":{
			"title":"<spring:message code='logic.capacity' />(MB)",
			"width":"25%"
		},
		"actualSpaceUsed":{
			"title":"<spring:message code='actual.capacity' />(MB)",
			"width":"25%"
		},
		"unit":{
...
function initTable(){
	opts_viewGrid = $("#fileStoreAddTable").comboTableGrid({
		headData : headData,
		ItemOp : "user-defined",
		dataId : "date",
	...
	$.fn.comboTableGrid.setItemOp = function(tableData, rowData, tdItem,
		...
		case "spaceUsed":
			...
			break;
		case "actualSpaceUsed":
			tdItem.find("p").prepend(rowData.actualSpaceUsed).parent().attr("title", rowData.actualSpaceUsed);
			break;
		case "unit":
			if(timeUnit == "year"){
...
function createView(){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/statistics/getFileStoreAddView/<c:out value='${treeNodeId}'/>",
		data : $("#addHistoryFileStoreForm").serialize(),
		error : function(data) {
		...
		success : function(data) {
			...
			$("#fileStoreAddTable").show();
			$("#fileStoreAddTable").setTableGridData(catalogData, opts_viewGrid);
			}
