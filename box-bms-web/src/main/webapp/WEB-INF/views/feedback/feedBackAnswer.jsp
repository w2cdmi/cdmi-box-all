	...
	<div class="pop-content">
		...
		<div class="detailBody">
			...
			<form class="form-horizontal" id="updateFeedBackForm"
				name="updateFeedBackForm">
				<div style="word-wrap:break-word;word-break:break-all">
					<b><spring:message code="feedback.detail.title" />：</b>${cse:htmlEscape(feedBackDetail.problemTitle)}
				</div>
				<div>
					<b><spring:message code="feedback.detail.type" />：</b>${cse:htmlEscape(feedBackDetail.problemType eq '1' ? '故障反馈':'意见建议')}
				</div>
				<div style="word-wrap:break-word;word-break:break-all">
					<b><spring:message code="feedback.detail.description" />：</b>${cse:htmlEscape(feedBackDetail.problemDescription)}
				</div>
				<div class="divFloatRight">
					<span class="customerFeedBack">${cse:htmlEscape(feedBackDetail.customerName)}</span>
					<span><fmt:formatDate
							value="${feedBackDetail.customerTwTime}"
				...
				<c:forEach items="${restFeedBackSubList}" var="feedbackSub">
					...
					<c:if
						test="${cse:htmlEscape(feedbackSub.isAnswer eq '1' ? true:false)}">
						...
						<div class="divFloatRight">
							...
							<span><fmt:formatDate value="${feedbackSub.updateTime}"
									pattern="yyyy-MM-dd HH:mm:ss" /></span>
						</div>
					...
					<c:if
						test="${cse:htmlEscape(feedbackSub.isAnswer eq '0' ? true:false)}">
						<div style="padding-top:40px;word-wrap:break-word;word-break:break-all">
				...
				<div>
					<div class="control-group">
						<label class="control-label" for=""
							style="float: left; width: 54px; padding-top: 5px; text-align: left; margin-right: 20px;"><em>*</em><b><spring:message
									code="feedback.detail.answer" />：</b></label>
						<div class="controls" style="margin-left: 60px;">
							<textarea id="answerDescription" name="answerDescription"
								style="margin: 0px; width: 490px; height: 160px;"><%-- ${cse:htmlEscape(feedBackDetail.answerDescription)} --%></textarea>
				...
				<input type="hidden" id="token" name="token" value="${token}" /> <input
					type="hidden" id="token" name="problemID"
					value="${feedBackDetail.problemID}" />
			</form>
	...
	<script type="text/javascript">
		...
		function submitModifyFeedBack() {
			...
			$
					.ajax({
						type : "POST",
						url : "${ctx}/feedback/uam/modify",
						data : $('#updateFeedBackForm').serialize(),
						error : function(request) {
							handlePrompt("error",
									'<spring:message code="feedback.answer.fail"/>');
						},
						success : function() {
							top.ymPrompt.close();
							top
									.handlePrompt("success",
											'<spring:message code="feedback.answer.success"/>');
							top.document.getElementById("feedBackOpenTabId")
									.click();
						}
					});
