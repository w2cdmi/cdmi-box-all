 ...
 *
 */
@Service("handleForAppDataMigrationClearData")
public class HandleForAppDataMigrationClearData implements BaseHandleCopyTask
{

    private static final Logger LOGGER = LoggerFactory.getLogger(HandleForAppDataMigrationClearData.class);
    ...
    private void handleObjectConflict(INode srcNode,CopyTask task)   
    {
        ...
        { 
            ...
            try{
                status =filesInnerManager.changeNodeObject(task,srcNode, objRef, false, true);
            }
            catch(Exception e)
            {
    ...
    public void handleCopyResult(CopyTask task, CopyTaskExeResult result)
    {
        ...
        // 创建新对象，然后替换srcNode的object
        ReplaceObjectStatus status = filesInnerManager.changeNodeObject(task,srcNode, obj, true, true);
        if(ReplaceObjectStatus.FINISH_STATUS == status)
        {
            // 更新迁移任务进度
            updateDataMigrationProcess(task);
