package com.huawei.sharedrive.app.openapi.domain.userlog;

import java.util.List;

public class UserLogListReq
{
    
    private String appId;
    
    private Long beginTime;
    
    private String clientAddress;
    
    private String clientDeviceName;
    
    private String clientDeviceSN;
    
    private String clientOS;
    
    private String clientType;
    
    private Byte clientTypeDb;
    
    private String clientVersion;
    
    private String detail;
    
    private Long endTime;
    
    private String level;
    
    private Byte levelDb;
    
    private Integer limit;
    
    private String loginName;
    
    private Long offset;
    
    private String type;
    
    private List<Integer> typeDb;
    
    private Long userId;
    
    public String getAppId()
    {
        return appId;
    }
    
    public Long getBeginTime()
    {
        return beginTime;
    }
    
    public String getClientAddress()
    {
        return clientAddress;
    }
    
    public String getClientDeviceName()
    {
        return clientDeviceName;
    }
    
    public String getClientDeviceSN()
    {
        return clientDeviceSN;
    }
    
    public String getClientOS()
    {
        return clientOS;
    }
    
    public String getClientType()
    {
        return clientType;
    }
    
    public Byte getClientTypeDb()
    {
        return clientTypeDb;
    }
    
    public String getClientVersion()
    {
        return clientVersion;
    }
    
    public String getDetail()
    {
        return detail;
    }
    
    public Long getEndTime()
    {
        return endTime;
    }
    
    public String getLevel()
    {
        return level;
    }
    
    public Byte getLevelDb()
    {
        return levelDb;
    }
    
    public Integer getLimit()
    {
        return limit;
    }
    
    public String getLoginName()
    {
        return loginName;
    }
    
    public Long getOffset()
    {
        return offset;
    }
    
    public String getType()
    {
        return type;
    }
    
    public Long getUserId()
    {
        return userId;
    }
    
    public void setAppId(String appId)
    {
        this.appId = appId;
    }
    
    public void setBeginTime(Long beginTime)
    {
        this.beginTime = beginTime;
    }
    
    public void setClientAddress(String clientAddress)
    {
        this.clientAddress = clientAddress;
    }
    
    public void setClientDeviceName(String clientDeviceName)
    {
        this.clientDeviceName = clientDeviceName;
    }
    
    public void setClientDeviceSN(String clientDeviceSN)
    {
        this.clientDeviceSN = clientDeviceSN;
    }
    
    public void setClientOS(String clientOS)
    {
        this.clientOS = clientOS;
    }
    
    public void setClientType(String clientType)
    {
        this.clientType = clientType;
    }
    
    public void setClientTypeDb(Byte clientTypeDb)
    {
        this.clientTypeDb = clientTypeDb;
    }
    
    public void setClientVersion(String clientVersion)
    {
        this.clientVersion = clientVersion;
    }
    
    public void setDetail(String detail)
    {
        this.detail = detail;
    }
    
    public void setEndTime(Long endTime)
    {
        this.endTime = endTime;
    }
    
    public void setLevel(String level)
    {
        this.level = level;
    }
    
    public void setLevelDb(Byte levelDb)
    {
        this.levelDb = levelDb;
    }
    
    public void setLimit(Integer limit)
    {
        this.limit = limit;
    }
    
    public void setLoginName(String loginName)
    {
        this.loginName = loginName;
    }
    
    public void setOffset(Long offset)
    {
        this.offset = offset;
    }
    
    public void setType(String type)
    {
        this.type = type;
    }
    
    public List<Integer> getTypeDb()
    {
        return typeDb;
    }
    
    public void setTypeDb(List<Integer> typeDb)
    {
        this.typeDb = typeDb;
    }
    
    public void setUserId(Long userId)
    {
        this.userId = userId;
    }
}
