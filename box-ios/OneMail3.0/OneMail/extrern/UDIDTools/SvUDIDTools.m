...
static const char kKeyChainUDIDAccessGroup[] = "248A5BM8T8.com.huawei.clouddrive";

@implementation SvUDIDTools

+ (NSString*)UDID
{
    NSString *udid = nil;//[SvUDIDTools getUDIDFromKeyChain];
    if (!udid) {
...
+ (NSString*)getUDIDFromKeyChain
{
   ...
   // NSString *bundleId=[[NSBundle mainBundle]bundleIdentifier];
    ...
    if (queryErr == errSecItemNotFound) {
        [SNLog Log:LInfo :@"KeyChain Item: %@ not found!!!", [NSString stringWithUTF8String:kKeychainUDIDItemIdentifier]];
    }
    else if (queryErr != errSecSuccess) {
       [SNLog Log:LInfo :@"KeyChain Item query Error!!! Error code:%ld", queryErr];
    }
    if (queryErr == errSecSuccess) {
       [SNLog Log:LInfo :@"KeyChain Item: %@", udidValue];
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([SvUDIDTools getUDIDFromKeyChain]) {        // there is item in keychain
        ...
        if (writeErr != errSecSuccess) {
           [SNLog Log:LInfo :@"Add KeyChain Item Error!!! Error Code:%ld", writeErr];
            
            [dictForAdd release];
            return NO;
        }
        else {
           [SNLog Log:LInfo :@"Add KeyChain Item Success!!!"];
            [dictForAdd release];
            return YES;
        }
    }
...
+ (BOOL)removeUDIDFromKeyChain
{
    ...
    if (deleteErr != errSecSuccess) {
        [SNLog Log:LInfo :@"delete UUID from KeyChain Error!!! Error code:%ld", deleteErr];
        [dictToDelete release];
        return NO;
    }
    else {
       [SNLog Log:LInfo :@"delete success!!!"];
    }
...
+ (BOOL)updateUDIDInKeyChain:(NSString*)newUDID
{
    ...
    if (queryResult) {
        ...
        if (updateErr != errSecSuccess) {
           [SNLog Log:LInfo :@"Update KeyChain Item Error!!! Error Code:%ld", updateErr];
           ...
           [SNLog Log:LInfo :@"Update KeyChain Item Success!!!"];
            [dictForQuery release];
            [dictForUpdate release];
            return YES;
        }
