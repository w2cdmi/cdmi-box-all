...
- (void)commonInit {
  ...
  //_shouldUpdateVisiblePagesWhileScrolling=YES;
  _updateCounts=0;
...
//    if (![self isDisplayingPageForIndex:_centerPageIndex+1]&&_centerPageIndex+1<self.numberOfPages) {
//      [self displayPageAtIndex:_centerPageIndex+1];
//    }
//    if (![self isDisplayingPageForIndex:_centerPageIndex-1]&&_centerPageIndex-1>=0) {
//      [self displayPageAtIndex:_centerPageIndex-1];
//    }
    ...
    if (shouldNotifyDelegate) {
      ...
      NSRange rangeOfVisiblePages ;
//      if (0 >= self.numberOfPages) {
//        rangeOfVisiblePages= NSMakeRange(0, 0);
//      }
      ...
      rangeOfVisiblePages= NSMakeRange(firstVisiblePageIndex, lastVisiblePageIndex - firstVisiblePageIndex + 1);
      for (NSUInteger pageIndex = rangeOfVisiblePages.location;
           pageIndex < NSMaxRange(rangeOfVisiblePages); ++pageIndex) {
        if (![self isDisplayingPageForIndex:pageIndex])
        {
...
[self updateVisiblePagesShouldNotifyDelegate:NO];
[self resetSurroundingPages];
    [_dataSource refreshFile:[self currentVisiblePageIndex]];
  if ([self.delegate respondsToSelector:_cmd]) {
...
-(void)reloadData:(int)currentPathIndex
{
    [self reloadData];
    [self moveToPageAtIndex:currentPathIndex animated:NO updateVisiblePagesWhileScrolling:NO];
}
- (void)reloadData {
  _animatingToPageIndex = -1;
...
- (BOOL)moveToPageAtIndex:(NSInteger)pageIndex animated:(BOOL)animated {
  return [self moveToPageAtIndex:pageIndex animated:animated updateVisiblePagesWhileScrolling:YES];
}
