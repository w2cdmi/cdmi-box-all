    ...
    for (id name in [self.cacheMap copy]) {
      ...
      if ([info hasExpired]) {
          dispatch_async(dispatch_get_main_queue(), ^{
              UIAlertView *alert=[ [UIAlertView alloc]initWithTitle:name message:@"superremove" delegate:nil cancelButtonTitle:@"cancel" otherButtonTitles:nil, nil];
              [alert show];
          });
        [self removeCacheInfoForName:name];
      }
...
- (void)reduceMemoryUsage {
  ...
  @synchronized(self) {
    ...
    [super reduceMemoryUsage];
      //UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"TITEL" message:[NSString stringWithFormat:@"%llu_%llu_%llu",self.numberOfPixels,self.maxNumberOfPixels,self.maxNumberOfPixelsUnderStress] delegate:nil cancelButtonTitle:@"cancel" otherButtonTitles:nil, nil];
      //[alert show];
