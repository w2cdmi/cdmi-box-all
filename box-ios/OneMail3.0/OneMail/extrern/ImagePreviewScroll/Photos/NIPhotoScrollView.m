...
@implementation NIPhotoScrollView {
  ...
  UITapGestureRecognizer * _singleTapGestuireRecongnizer;
}
...
- (void)didSingleTap:(UITapGestureRecognizer *)tapGesture {
    [[NSNotificationCenter defaultCenter]postNotificationName:@"com.huawei.preview.singleTap" object:[NSNumber numberWithBool:self.SingleTapTaped]];
   
    self.SingleTapTaped=!self.SingleTapTaped;
...
- (void)setSingleTapToZoomIsEnabled:(BOOL)enabled {
    if (enabled && nil == _singleTapGestuireRecongnizer) {
        _singleTapGestuireRecongnizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(didSingleTap:)];
        _singleTapGestuireRecongnizer.numberOfTapsRequired = 1;
        [self addGestureRecognizer:_singleTapGestuireRecongnizer];
    }
    
    _singleTapGestuireRecongnizer.enabled = enabled;
}
- (BOOL)isDoubleTapToZoomEnabled {
  return [_doubleTapGestureRecognizer isEnabled];
}
- (BOOL)isSingleTapToZoomEnabled {
    return [_singleTapGestuireRecongnizer isEnabled];
}
