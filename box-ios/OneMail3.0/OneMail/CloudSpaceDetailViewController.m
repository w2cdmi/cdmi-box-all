//
//  CloudSpaceDetailViewController.m
//  OneMail
//
...
#import "CloudSpaceDetailTableViewCell.h"
...
@interface CloudSpaceDetailViewController ()<UITableViewDelegate,UITableViewDataSource,NSFetchedResultsControllerDelegate,UIActionSheetDelegate,UITextFieldDelegate>
...
@implementation CloudSpaceDetailViewController
...
- (id)initWithFile:(File*)file {
    ...
    if (self) {
        ...
        self.cloudSearchCancelButton.imageView.frame = CGRectMake(11, 11, 25, 22);
        [self.cloudSearchCancelButton setTitle:getLocalizedString(@"Undo", nil) forState:UIControlStateNormal];
//        [self.cloudSearchCancelButton setImage:[UIImage imageNamed:@"ic_nav_back_nor"] forState:UIControlStateNormal];
...
//        [self.cloudBackButton setTitle:@"取消" forState:UIControlStateNormal];
...
- (void)viewDidLoad {
    ...
    if (!self.crumbFileIds) {
        ...
        [self.crumbFileViewControllers addObject
         :self];
    }
    ...
    [self.cloudFileTableView registerClass:[CloudSpaceDetailTableViewCell class] forCellReuseIdentifier:@"CloudSpaceDetailTableViewCell"];
...
//    self.cloudFileTableView.tableHeaderView = self.cloudFileRefreshController;
    ...
    [self.cloudSearchTableView registerClass:[CloudSpaceDetailTableViewCell class] forCellReuseIdentifier:@"CloudSpaceDetailTableViewCell"];
    ...
    if (self.cloudTitleView.titleStyle == CloudTitleMySpaceStyle) {
        ...
        handleRevognizer.direction = UISwipeGestureRecognizerDirectionLeft;
...
//                if ([strongSelf.cloudFileRefreshController isRefreshing]) {
//                    [strongSelf.cloudFileRefreshController endRefreshing];
//                }
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CloudSpaceDetailTableViewCell *cell = (CloudSpaceDetailTableViewCell*)[tableView dequeueReusableCellWithIdentifier:@"CloudSpaceDetailTableViewCell"];
    if (cell == nil) {
        cell = [[CloudSpaceDetailTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"CloudSpaceDetailTableViewCell"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CloudSpaceDetailTableViewCell *cell = (CloudSpaceDetailTableViewCell*)[tableView cellForRowAtIndexPath:indexPath];
    File *file = [fetchController objectAtIndexPath:indexPath];
    if (self.cloudSelectState) {
        if ([self.cloudSelectedArray containsObject:file]) {
            cell.fileSelected = NO;
        ...
        if ([file isFolder]) {
            if (self.cloudSearchState) {
                CloudSpaceDetailViewController *child = [[CloudSpaceDetailViewController alloc] initWithFile:file];
                child.teamSpace = self.teamSpace;
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView editingStyleForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([tableView isEqual:_cloudFileTableView]) {
...
- (CloudFileHandleMenuView*)cloudFileHandleMenu {
    if (!_cloudFileHandleMenu) {
        ...
        CloudFileHandleMenuCell *storgeToMySpaceMenuCell = [[CloudFileHandleMenuCell alloc] initWithTitle:getLocalizedString(@"CloudFileTransmitMySpace", nil) target:self action:@selector(fileSaveToMySpace)];
        ...
        [_cloudFileHandleMenu setMenuCells:@[storgeToMySpaceMenuCell,moveMenuCell,renameMenuCell,viewVersionMenuCell]];
    }
...
#pragma mark File Premission
- (void)fileUploadPremission:(void(^)(BOOL permission))completion {
    [self.teamSpace getMemberInfoWithId:self.teamSpace.teamRelationId succeed:^(id retobj) {
        NSString *role = [retobj objectForKey:@"role"];
        if ([role isEqualToString:@"auther"] ||
            [role isEqualToString:@"editor"] ||
            [role isEqualToString:@"uploadAndView"] ||
            [role isEqualToString:@"uploader"]) {
            completion(YES);
        } else {
            completion(NO);
        }
...
- (void)fileDownloadPremission:(void(^)(BOOL permission))completion {
    [self.teamSpace getMemberInfoWithId:self.teamSpace.teamRelationId succeed:^(id retobj) {
        NSString *role = [retobj objectForKey:@"role"];
        if ([role isEqualToString:@"auther"] ||
            [role isEqualToString:@"editor"] ||
...
- (void)fileEditPremission:(void(^)(BOOL permission))completion {
    [self.teamSpace getMemberInfoWithId:self.teamSpace.teamRelationId succeed:^(id retobj) {
        NSString *role = [retobj objectForKey:@"role"];
        if ([role isEqualToString:@"auther"] ||
            [role isEqualToString:@"editor"]) {
...
- (void)fileDownload:(UIButton*)button {
    ...
    [self fileDownloadPremission:^(BOOL permission) {
        if (permission) {
            button.selected = YES;
            AppDelegate *appDelegate = [UIApplication sharedApplication].delegate;
...
- (void)fileUpload {
    [self fileUploadPremission:^(BOOL permission) {
        if (permission) {
...
- (void)fileCreateFolder {
    ...
    [self.navigationController pushViewController:creatFolderVC animated:YES];
}
...
- (UIView*)cloudSearchNavigationBarView {
    if (!_cloudSearchNavigationBarView) {//15+22+2
        ...
        self.cloudSearchNavigationBarTextField = [[UITextField alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.cloudSearchNavifationBarImageView.frame)+4, 7, CGRectGetWidth(_cloudSearchNavigationBarView.frame)-CGRectGetMaxX(self.cloudSearchNavifationBarImageView.frame)-4-10,22)];
        ...
        self.cloudSearchNavigationBarTextField.font = [UIFont systemFontOfSize:14.0f];
...
- (void)controller:(NSFetchedResultsController *)controller didChangeObject:(id)anObject atIndexPath:(NSIndexPath *)indexPath forChangeType:(NSFetchedResultsChangeType)type newIndexPath:(NSIndexPath *)newIndexPath {
    CloudSpaceDetailTableViewCell *cell = (CloudSpaceDetailTableViewCell *)[self.cloudFileTableView cellForRowAtIndexPath:indexPath];
    UITableView *tableView;
    if (controller == self.cloudSearchFetchController) {
        tableView = self.cloudSearchTableView;
    } else {
    ...
    switch (type) {
        case NSFetchedResultsChangeInsert:
            ...
            //            if (controller != self.cloudSearchFetchController && controller.fetchedObjects.count == 1) {
            //                dispatch_async(dispatch_get_main_queue(), ^{
            //                    self.cloudFileNullView.hidden = YES;
            //                });
            //            }
        ...
        case NSFetchedResultsChangeDelete:
            ...
            //            if (controller != self.cloudSearchFetchController && controller.fetchedObjects.count == 0) {
            //                dispatch_async(dispatch_get_main_queue(), ^{
            //                    self.cloudFileNullView.hidden = NO;
            //                });
            //            }
