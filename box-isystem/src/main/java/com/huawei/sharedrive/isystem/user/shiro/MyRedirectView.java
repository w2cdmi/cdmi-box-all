 ...
 * HttpServletResponse's <code>sendRedirect</code> method, which is what actually does the
 ...
 * considered relative to the web application root. Since most web apps will never know or
 * care what their context path actually is, they are much better off setting this flag to
 * true, and submitting paths which are to be considered relative to the web application
 * root.
 * <p/>
 * Note that in a Servlet 2.2 environment, i.e. a servlet container which is only
 * compliant to the limits of this spec, this class will probably fail when feeding in
 * URLs which are not fully absolute, or relative to the current request (no leading "/"),
 * as these are the only two types of URL that <code>sendRedirect</code> supports in a
 * Servlet 2.2 environment.
 * <p/>
...
public class MyRedirectView
{
     ...
     * i.e. taken as-is. If true, the context path will be prepended to the URL in such a
     * case.
     * 
     ...
     * i.e. delegate to <code>HttpServletResponse.sendRedirect</code>. Turning this off
     * will send HTTP status code 303, which is the correct code for HTTP 1.1 clients, but
     * not understood by HTTP 1.0 clients.
     * <p>
