    ...
    $.fn.extend({
        'ui.Page': function () {
            ...
            function datagridLoad() {
                datagrid.load(_ownerId, _folderId, _pageNumber, _pageSize, _orderField, _order, _name, _isSearch,_receiveType)
            }
            // 移动文件
            function showMoveToDialog(node) {
                ...
                var folderChooser = $("#copyToFolderChooserDialog").FolderChooser({
                        title:"移动“"+ names +"”到",
                        exclude: function (r) {
                            return (r !== undefined && r.ownedBy === node.ownedBy && r.id === node.id) || r.type == -7;
                        },
