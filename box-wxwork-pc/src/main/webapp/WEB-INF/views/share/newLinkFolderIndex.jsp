...
<head>
    ...
    <link rel="stylesheet" href="${ctx}/static/skins/default/css/video-js.css">
    <link rel="stylesheet" href="${ctx}/static/skins/default/css/viewer.min.css">
...
<div class="box">
    ...
    <div id="downloadDiv" style="display: none; width: 100%; height: 1.7rem;">
        ...
        <div class="fs-view-file-list" id="fileListWrapper" style="background-size: 5rem 5rem; top: 5.2rem">
...
<script id="fileTemplate" type="text/template7">
    <div class="file line-content" style="border-bottom: 1px solid #E4E1E1;">
        <div class="file-info">
            {{#js_compare "this.imgPath==null"}}
            <div style="float: left;" class="img {{divClass}}"></div>
            {{else}}
            <div style="float: left;" class="img {{divClass}}" style="background:url({{imgPath}}) no-repeat center center;">
                <img data-index="{{num}}" data-origina='{{imgSrc}}' src='{{imgSrc}}' style="display: none" alt="{{name}}"/>
            </div>

            {{/js_compare}}
            <div id="file_{{id}}" onclick="optionInode(this)">
...
<script type="text/javascript">
    ...
    var ctx = "${ctx}"
    var host = "";//
    var permissionFlag = getLinkPermission(ownerId, parentId);
    ...
    function uploadFile() {
        ...
        for (var i = 0; i < oFiles.length; i++) {
            ...
            reciveupLoad(params, file);
        }
    }

    function reciveupLoad(params, file) {
    ...
    function openFolder(node) {
        ...
        var nodePermission = getLinkPermission(ownerId, parentId);
    ...
    function optionInode(t) {
        ...
        if (node.type == 0) {
            ...
            previewOrDownloadLinkFile(node, t)
        }
    }

    function previewOrDownloadLinkFile(node, t) {
        var ownerId = node.ownedBy || node.ownerId;
        var nodeId = node.id;

        var preview = isPreviewable(node.name);
        var video = isVideo(node.name)
        var nodePermission = getLinkPermission(ownerId, nodeId);

        if (nodePermission == undefined || nodePermission == null || nodePermission["preview"] != 1) {
            $.Tost("您没有预览权限");
            return;
        }

        if (preview) {
            gotoPageOpen('/p/preview/' + linkCode + '?ownerId=' + ownerId + '&nodeId=' + nodeId) //此处使用/p绝对路径，不使用ctx路径，相关的路径转换由nginx负责完成

        } else {
            if (nodePermission == undefined || nodePermission == null || nodePermission["download"] != 1) {
                $.Tost("该文件不支持预览，您可以联系管理员给予下载权限");
                return;
            }
            if (isImg(node.name)) {
                var dataIndex = $(t).prev().find("img").attr("data-index")
                if (viewer !== null) {
                    viewer.destroy();
                }
                viewer = new Viewer(document.getElementById('fileList'), {
                    url: 'data-original',
                    shown: function () {
