...
gulp.task('build-js', function () {
    return gulp.src(['src/main/webapp/static/**/*.js'])
        ...
        .pipe(rev())
        .pipe(gulp.dest('target/cloudapp-wx-' + options.profile + '/static'))
        .pipe(rev.manifest())
...
gulp.task('build-jsp-js', function () {
    return gulp.src(['target/rev/js/*.json', 'target/cloudapp-wx-' + options.profile + '/WEB-INF/views/**/*.jsp'], {base: 'target/cloudapp-wx-' + options.profile + '/WEB-INF/views'})
        .pipe(revCollector({
            replaceReved: true,
            dirReplacements: {}
        }))
        .pipe(gulp.dest('target/cloudapp-wx-' + options.profile + '/WEB-INF/views'));
});

gulp.task('build-jsp-css', function () {
    return gulp.src(['target/rev/css/*.json', 'target/cloudapp-wx-' + options.profile + '/WEB-INF/views/**/*.jsp'], {base: 'target/cloudapp-wx-' + options.profile + '/WEB-INF/views'})
        .pipe(revCollector({
            replaceReved: true,
            dirReplacements: {}
        }))
