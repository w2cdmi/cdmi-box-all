...
(function() {
	...
	var mkBtn = function(txt, sign, autoClose, id) {
		...
		return {
			id: id,
			/*修改按钮标签为button*/
			html: "<button type='button' id='" + id + "' onclick='ymPrompt.doHandler(\"" + sign + "\"," + autoClose + ")' class='btn' >" + txt + "</button>"
		};
	};
	/*生成按钮组合的html*/
	...
	var dftCfg = {
		...
		maskAlphaColor: 'black',	//遮罩透明色，默认黑色
		maskAlpha: 1,			//遮罩透明度，默认0.1
		...
		showShadow: true,		//是否显示消息框的阴影，默认显示
	...
	(function() {
		...
		/*阴影*/
		"<div id='ym-shadow' style='position:absolute;z-index:10000;display:none'></div>"].join('');
		...
		var resizeMask = function(noDelay) {
			...
			cur_state == 'min' ? doMin() : cur_state == 'max' ? doMax() : setWinPos(); //最大化最小化状态还原
		};
		...
		var maskVisible = function(visible) {
			...
			/* ie8偶现遮罩无法半透明的问题，因此修改为图片背景的方式  modify start */
			if(curCfg.maskAlphaColor == "gray"){
				maskLevel.className = "ymPrompt-graymask";
			}else{
				maskLevel.className = "ymPrompt-blackmask";
			}
			/* modify end */
		...
		var winVisible = function(visible) {
			...
			var ifmStyle = 'position:absolute;width:100%;height:100%;top:0;left:0;';	//modify by zhanpengchong z90005455 (兼容ie8在二级弹出框样式透明的问题)
			ym_body.innerHTML = !curCfg.iframe ? ('<table style="height:100%; width:100%;" class="' + curCfg.msgCls + '"><tbody><tr><td>' + curCfg.message + '</td></tr></tbody></table>') : "<iframe style='" + ifmStyle + "' border='0' allowtransparency='true' frameborder='0' src='" + curCfg.message + "'></iframe><div style='" + ifmStyle + ";background:#fff;opacity:0;filter:alpha(opacity=0);display:none'></div>"; //内容
			(function(el, obj) {
