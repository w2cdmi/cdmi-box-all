...
function listFile(folderId, page) {
    ...
    __page = page || 1;
    var permission = getNodePermission(sharedownerId,parentId);
    var p = 0;
...
function openFolder(node) {
    ...
    var nodePermission = getNodePermission(sharedownerId,parentId);
...
function createFolder(newName, callback) {
    ...
    $.ajax({
        type: "POST",
        url: host + "/ufm/api/v2/folders/" + sharedownerId,
...
function deleteFiles(ownerId, nodeId) {
	 $.ajax({
         type: "DELETE",
         url: host + "/ufm/api/v2/nodes/" + ownerId + "/" + nodeId,
         error: function (data) {
