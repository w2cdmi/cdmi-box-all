...
function getFileIcon(row) {
    if (row.type <= 0) {
        ...
        if (row.isSecret) {
            ...
            return '<i class="' + (row.shareStatus == 1 ? 'ico-sharefolder' : 'ico-folder') + '"></i>'
        }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var info = {
                ...
                "size": node.size,
                "complete": 100,
                "createdAt": getFormatDate(new Date(), "yyyy/MM/dd")
            };
...
function downloadFileByNodeId(ownerId, nodeId) {
    var nodePermission = getNodePermission(ownerId, nodeId, curUserId);
 ...
 * @param ownerId
 */
function downloadFileByNodeIdAndOwnerId(ownerId, nodeId, name) {
    var preview = isPreviewable(name)
    if (preview) {
        gotoPageOpen(ctx + '/files/gotoPreview/' + ownerId + '/' + nodeId)
    } else {
        $.ajax({
